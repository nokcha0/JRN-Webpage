...
function buildSelectionWindow() {
  ...
  <div class="continent-selector">
    <div class="continent-heading">CONTINENTS</div><hr><br>
  `;
  for (let i = 0; i < regions[0].length; i++) {
    regionSelectionHtml += `
    <div class="continent-element" id="${regions[0][i]}">
      <span>${localeString(regions[0][i])}</span>
    </div>
  `;
  }
  regionSelectionHtml += `
  ...
  regionSelectionHtml += `
  <div class="continent-selector">
    <div class="continent-heading">OTHERS</div><hr><br>
  `;
  for (let j = 0; j < regions[1].length; j++) {
    if (j === 0) {
      regionSelectionHtml += `
      <div class="continent-element" id="${regions[1][0]}">${localeString(regions[1][0])}</div>
    `;
    } else {
      regionSelectionHtml += `
      <div class="continent-element" id="${regions[1][j]}">
        <img src="./img/flags/${regions[1][j]}.svg" alt="Flag icon" class="thumbnails">
        <span>${localeString(regions[1][j])}</span>
      </div>
    `;
    }
  }
