...
const encodeText = document.querySelector(".encode");
const encodeResult = document.querySelector(".result-encode"); 

const decodeText = document.querySelector(".decode");
const decodeResult = document.querySelector(".result-decode");

encodeText.addEventListener("input", () => {
    encodeResult.value = korTypeToEng(encodeText.value); 
});

decodeText.addEventListener("input", () => {  
    decodeResult.value = engTypeToKor(decodeText.value);
});
...
// Encode Copy Button
document.querySelector(".encode-copy").addEventListener("click", () => {

    const encodeResultElem = document.querySelector(".result-encode");
    
    encodeResultElem.select();
  
    const copySuccess = document.execCommand("copy");
  
    if(!copySuccess) {
      alert("Failed to copy encode result");
    }
  });
  
  // Decode Copy Button 
  document.querySelector(".decode-copy").addEventListener("click", () => {
  
    const decodeResultElem = document.querySelector(".result-decode");
     
    decodeResultElem.select();
     
    const copySuccess = document.execCommand("copy");
  
    if(!copySuccess) {
      alert("Failed to copy decode result"); 
    }  
  });
